...
class CiCommand(StewBaseCommand):
    ...
    arguments = [Argument("project-name", required=False, is_list=False, description="The name of the project to run CI checks on. If not provided, all projects will be checked.")]
    ...
    options = [
        ...
        Option("check", is_list=True, flag=False, description="Specify which checks to run (e.g., pytest, black, mypy)."),
        Option("skip", is_list=True, flag=False, description="Specify which checks to skip."),
        Option("quick", description="Do not call 'poetry install --sync' before testing. A quick preset may be defined in the pyproject file."),
        ...
        Option("extra", is_list=True, flag=False, description="Additional extras to include when testing."),
